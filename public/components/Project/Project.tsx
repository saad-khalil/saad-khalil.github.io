...
import {FancyLink} from "@/public/components/FancyLink/FancyLink";
...
export default function index({index, title, description, url, setModal}) {
    ...
    return (
        //@ts-ignore
        <FancyLink href={url} onMouseEnter={() => {setModal({active: true, index})}} onMouseLeave={() => {setModal({active: false, index})}} className={`${styles.project} ${isClicked ? styles.noHover : ''}`}  onClick={handleTransition} >
            <h2>{title}</h2>
            <p>{description}</p>
        </FancyLink>
    )
